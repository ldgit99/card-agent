    ...
    if (hasOpenAIKey()) {
      try {
        const response = await parseStructuredResponse({
          ...
          system:
            "당신은 교사의 수업 시뮬레이션 관찰자다. 활동별로 AI 과의존, 깊이 있는 학습 부족, 근거 없는 판단, 책임 주체 불명확, 교사의 최종 판단 부재, 심리적 안전 저해, 설계-행동 불일치, 참여 편중, 평가 불일치를 탐지하라. 각 위험에는 활동명, 초점 영역, 학생에게 미치는 영향, 관찰 신호, 즉시 가능한 교사 개입을 포함하라.",
          payload: {
            design,
            turns,
          },
