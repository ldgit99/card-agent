    ...
    <main className="reportShell">
      <section className="reportToolbar printHidden">
        <div>
          ...
          <p className="panelHint">브라우저에서 바로 읽고, `PDF로 저장`으로 인쇄 대화상자를 열 수 있습니다.</p>
        </div>
      ...
      <section className="reportSection">
        ...
        <div className="reportGrid reportGridThree">
          <article className="reportCard">
            <h3>성취기준</h3>
            <ul>
              {report.design.achievementStandards.length ? report.design.achievementStandards.map((item) => <li key={item}>{item}</li>) : <li>없음</li>}
            </ul>
          </article>
          <article className="reportCard">
            <h3>학습 목표</h3>
            <ul>
              {report.design.learningGoals.length ? report.design.learningGoals.map((item) => <li key={item}>{item}</li>) : <li>없음</li>}
            </ul>
          </article>
          <article className="reportCard">
            <h3>설계 정보</h3>
            <ul>
              ...
              <li>생성일 {formatDateTime(report.design.createdAt)}</li>
              <li>수정일 {formatDateTime(report.design.updatedAt)}</li>
            </ul>
          </article>
        </div>
                  ...
                  <tr key={activity.id}>
                    ...
                    <td>
                      <div className="reportChipWrap">
                        {cards.teacher.length ? cards.teacher.map((item) => <span key={`${activity.id}-${item}`} className="reportChip">{item}</span>) : <span className="reportChip reportChipMuted">없음</span>}
                      </div>
                    </td>
                    <td>
                      <div className="reportChipWrap">
                        {cards.ai.length ? cards.ai.map((item) => <span key={`${activity.id}-${item}`} className="reportChip">{item}</span>) : <span className="reportChip reportChipMuted">없음</span>}
                      </div>
                    </td>
                  </tr>
                );
      ...
      {report.analysis ? (
        <section className="reportSection">
          <div className="panelHeader">
            <div>
              <p className="sectionTag">Analysis</p>
              <h2>설계 분석</h2>
            </div>
            <span className="engineBadge">{report.analysis.engine}</span>
          </div>
          <article className="reportCard reportSummaryCard">
      ...
      {report.scenario ? (
        <section className="reportSection">
          <div className="panelHeader">
            <div>
              <p className="sectionTag">Scenario</p>
              <h2>모의 수업 시나리오</h2>
            </div>
            <span className="engineBadge">{report.scenario.engine}</span>
          </div>
          <div className="reportGrid reportGridThree">
            ...
            {report.scenario.episodes.map((episode, index) => (
              <article key={episode.id} className="reportCard">
                <div className="reportEpisodeHead">
                  <span className="reportChip">Episode {index + 1}</span>
                  <span className="reportChip">{episode.lens}</span>
                </div>
                <h3>{episode.title}</h3>
            ...
            {report.turns.map((turn) => (
              <article key={turn.id} className="reportCard">
                ...
                <ul className="reportDetailList">
                  ...
                  <li><strong>놓친 기회</strong><span>{turn.missedOpportunities.join(" / ") || "없음"}</span></li>
      ...
      <section className="reportSection">
        ...
        {report.risks.length ? (
          ...
          <p className="emptyPanelText">포착된 주요 위험이 없습니다.</p>
        )}
      </section>
      ...
      <section className="reportSection">
        ...
        <div className="reportStack">
          ...
          <article className="reportCard">
            <h3>종합 메모</h3>
            <div className="reportAnswerBlock">{report.summary || "입력 없음"}</div>
          </article>
          <article className="reportCard">
            <h3>다음 수정 체크리스트</h3>
            <ul>
              {report.nextRevisionNotes.length ? report.nextRevisionNotes.map((note) => <li key={note}>{note}</li>) : <li>없음</li>}
            </ul>
          </article>
        </div>
      </section>
